...
use giter_utils::types::{
    contribution::CommitStatistic, git_data_provider::GitDataProvider, status::WorkStatus,
};
...
pub fn repo_single_emit(event: Arc<Event>) {
    ...
    for (repo_path, _) in map.iter() {
        ...
        if repo_set.contains(&path) {
            app.emit(
                &format!("giter://repo_single_emit:{}", repo_path.replace("\\", "/")),
                (),
            )
            .expect("TODO: panic message");
        }
...
pub fn emit_branch_contribution(key: &str, value: anyhow::Result<Vec<CommitStatistic>>) {
    ...
    if let Err(e) = value {
        let _ = app.emit(
            &format!("giter://branch_contribution/{}", key),
            e.to_string(),
        );
    } else {
        let _ = app.emit(
