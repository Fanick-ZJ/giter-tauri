...
use crate::{types::{cache::RepoPath, store::{self, Repository}}, utils::{db::conn_db, dirs::{self, repo_default_alias}}};
...
impl GitStore {
  ...
  fn is_stored(&self, path: &str) -> bool {
    ...
    let count: i32 = stmt.query_row([path], |row| {
      row.get::<_, i32>(0)
    }).unwrap();
    count > 0
  }

  pub fn insert_repo(&self, path: &str, alias: &str, has_watch: bool, order: i32, top: bool) -> Result<store::Repository, String> {
  ...
  pub fn add_repo(&self, path: String, alias: Option<String>, has_watch: Option<bool>, order: Option<i32>, top: Option<bool>) -> Result<store::Repository, String> {
    // 查询是否存在;
  ...
  pub fn update_repo(&self, repo: store::Repository) -> Result<(), String> {
    let sql = "update repository set path=?1, alias=?2, has_watch=?3, `order`=?4, top=?5 where id=?6";
    let conn = conn_db(self.path.clone()).unwrap();
    let stmt = conn.execute(sql, params![repo.path, repo.alias, repo.has_watch, repo.order, repo.top, repo.id]);
    match stmt {
      Ok(_) => {
